...
grid.arrange(p_both$scat.plot, 
             ...
             ncol = 2,
             top = "TCGA-PAAD Clustering and Survival")


# Cluster and Plot Inflam only
kw3 <- list(INFLAM = c("INFLAMMATORY",
                       "INFLAMMATION"))
sets3 <- kwsearch(keywords = kw3,
                  data_genes = data$data_genes)
pcs3 <- findPC(data,sets3)
clust3 <- clusterPC(data,pcs3,n_clust)
surv3 <- survMax(clust3,
                 data$days_to_death,
                 data$vital_status,
                 deg_freedom = df)
p_inf <- 
